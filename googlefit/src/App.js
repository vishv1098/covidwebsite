...
class App extends React.Component {
 ...
 dataSource = async () => {
    await axios.get('https://www.googleapis.com/fitness/v1/users/me/dataSources',{
      headers: {
        'Authorization': 'Bearer ya29.a0AfH6SMDxrjq3x05G7NuBFczSUMpefN1klipWmzRGRbKW-g5Kz0l7H9ga9hF-eoonr059it7BM9UkH9z5LfNmUrUugrr8ky8-qfTZnECVCbU4weXbdeO9zEzlcFNtzEix8Ta_3h64GQvyyYj4vAUeQYlAEC-UIIOHHAYvX8HtF4Q',
      }
    }).then((resp) => {
  ...
  onStep = async () => {
    ...
    await axios.get('https://www.googleapis.com/fitness/v1/users/me/dataSources/'+this.state.step_count_token+'/datasets/'+result1+'000000-'+result2+'000000',{
      ...
      if(array.length === 0) {
        ...
        //console.log(this.state.dataset_step)
        var uniqueItems = Array.from(new Set(items));
        var str = "time," + uniqueItems.toString();
        var columns = str.split(",");
        var times = [];
  ...
  //       let val = array[q]["value"][0]["intVal"];
  //       if(val === 110 || val === 109){
  //        await this.onCreateData(stime,end);
  //       }
  //      }    
  //    });
  // }
  ...
  onCalories = async () => {
    ...
    await axios.get('https://www.googleapis.com/fitness/v1/users/me/dataSources/'+this.state.calories_token+'/datasets/'+result1+'000000-'+result2+'000000',{
      ...
      if(array.length === 0) {
        ...
        //console.log(this.state.dataset_calories)
        var uniqueItems = Array.from(new Set(items));
        var str = "time," + uniqueItems.toString();
        var columns = str.split(",");
        var times = [];
  ...
  onActivity = async () => {
   ...
   // console.log(this.state.token)
                ...
                for (var q = 0; q < array.length; q++) {
                  ...
                  let val = array[q]["value"][0]["fpVal"];
                   /* console.log(stime)
                    ...
                    // dA_activity.push(obj);
                    // items_activity.push(date.toLocaleDateString().slice(0, -5));
                   this.setState({ 
    ...
    // console.log(this.state.dA_activity)
    // console.log(this.state.items_activity)
    // var uniqueItems = Array.from(new Set(this.state.items_activity));
    // console.log(uniqueItems)
    // var str = "time," + uniqueItems.toString();
    // var columns = str.split(",");
    // var times = [];
  ...
  ondownload = async () => {
      ...
      FileDownload(this.state.total_data_step,'step-count.csv')
      FileDownload(this.state.total_data_calories,'calories.csv')
      FileDownload(this.state.total_data_distance,'distance.csv')
      await this.createDataActivity();
     FileDownload(this.state.total_data_activity,'activity.csv')
 }
